// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/maintenance/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for maintenance records on a vehicle.
//   Shows annual spend, monthly bar chart, and a record list
//   for all records of type "maintenance".
//
// Design:
//   Fetches records via the generic records endpoint filtered
//   by type=maintenance. Client-side aggregation builds the
//   monthly breakdown and stat totals. Mirrors the expenses
//   page pattern — same stat cards, same bar chart, same year
//   selector. No inline add form; users add via the Records page.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/maintenance
// ============================================================
...
export default function MaintenancePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await apiFetch(
        `/api/v1/accounts/${accountId}/vehicles/${id}/records?type=maintenance&page_size=200`
      );
      if (res.ok) {
  ...
  const monthlyAvg   = Math.round(annualSpend / (year === CURRENT_YEAR ? Math.max(new Date().getMonth() + 1, 1) : 12));
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Maintenance</h1>
              <select
          ...
          <Card>
            ...
            <div className="an-rec-list">
              {yearRecords.length === 0 && (
                <p style={{ color: "var(--colour-text-muted)", fontSize: "var(--text-sm)" }}>No records for {year}.</p>
              )}
              {yearRecords.map((r) => (
