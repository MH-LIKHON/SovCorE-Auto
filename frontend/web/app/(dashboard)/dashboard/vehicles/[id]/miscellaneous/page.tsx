// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/miscellaneous/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for miscellaneous records on a vehicle.
//   Shows annual spend, monthly bar chart, and a record list
//   for all records of type "custom" (labelled Miscellaneous).
//
// Design:
//   Fetches records via the generic records endpoint filtered
//   by type=custom. Client-side aggregation builds the monthly
//   breakdown and stat totals. Mirrors the expenses page pattern.
//   No inline add form; users add via the Records page.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/miscellaneous
// ============================================================
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await apiFetch(
        `/api/v1/accounts/${accountId}/vehicles/${id}/records?type=custom&page_size=200`
      );
    ...
    <div className="rec-shell">
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Miscellaneous</h1>
              <select className="rpt-year-select" value={year} onChange={(e) => setYear(Number(e.target.value))}>
                {yearOptions.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <p className="rec-sub">Miscellaneous records and costs not covered by other categories.</p>
          </div>
      ...
      {!loading && !hasData && (
        <Card><div className="rec-empty">
          <p>No miscellaneous records yet. Add records via the Records page.</p>
          <Link href={`/dashboard/vehicles/${id}/records`} className="rec-btn rec-btn--primary">Add record</Link>
        </div></Card>
      )}
