// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/parking/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for parking records on a vehicle.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/parking
// ============================================================
...
export default function ParkingPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await apiFetch(`/api/v1/accounts/${accountId}/vehicles/${id}/records?type=parking&page_size=200`);
  ...
  return (
    <div className="rec-shell">
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Parking</h1>
              <select className="rpt-year-select" value={year} onChange={(e) => setYear(Number(e.target.value))}>
                {yearOptions.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <p className="rec-sub">Parking charges and fees for this vehicle.</p>
          </div>
          <Link href={`/dashboard/vehicles/${id}/records?type=parking`} className="rec-btn rec-btn--primary rec-btn--icon" title="Add record">+</Link>
        </div>
      </header>
      ...
      {!loading && !hasData && (
        <Card><div className="rec-empty"><p>No parking records yet. Add records via the Records page.</p></div></Card>
      )}
