// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/tax/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for road tax records on a vehicle.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/tax
// ============================================================
...
export default function TaxPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await apiFetch(`/api/v1/accounts/${accountId}/vehicles/${id}/records?type=tax&page_size=200`);
  ...
  return (
    <div className="rec-shell">
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Tax</h1>
              <select className="rpt-year-select" value={year} onChange={(e) => setYear(Number(e.target.value))}>
                {yearOptions.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <p className="rec-sub">Road tax payment history and costs for this vehicle.</p>
          </div>
          <Link href={`/dashboard/vehicles/${id}/records?type=tax`} className="rec-btn rec-btn--primary rec-btn--icon" title="Add record">+</Link>
        </div>
      </header>
      ...
      {!loading && !hasData && (
        <Card><div className="rec-empty"><p>No tax records yet. Add records via the Records page.</p></div></Card>
      )}
