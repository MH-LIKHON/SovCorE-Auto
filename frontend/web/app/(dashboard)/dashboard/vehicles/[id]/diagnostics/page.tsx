...
//
// Purpose:
//   Analytics page for diagnostics records on a vehicle.
//   Shows annual spend, monthly bar chart, and a record list
//   for all records of type "diagnostics".
//
// Design:
//   Fetches records via the generic records endpoint filtered
//   by type=diagnostics. Client-side aggregation builds the
//   monthly breakdown and stat totals. Mirrors the maintenance
//   page pattern exactly.
//
// Consumed by:
...
export default function DiagnosticsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await apiFetch(`/api/v1/accounts/${accountId}/vehicles/${id}/records?type=diagnostics&page_size=200`);
  ...
  return (
    <div className="rec-shell">
      <header className="rec-head">
        <div className="rec-head__row">
          <div>
            ...
            <p className="rec-sub">Vehicle diagnostic reports, fault codes, and associated costs.</p>
          </div>
          <Link href={`/dashboard/vehicles/${id}/records?type=diagnostics`} className="rec-btn rec-btn--primary rec-btn--icon" title="Add record">+</Link>
        </div>
      </header>
      ...
      {!loading && !hasData && (
        <Card><div className="rec-empty"><p>No diagnostics records yet. Add records via the Records page.</p></div></Card>
      )}
