// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/mot/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for MOT records on a vehicle.
//   Shows annual spend, monthly bar chart, and a record list
//   for all records of type "mot".
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/mot
// ============================================================
...
export default function MotPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await apiFetch(`/api/v1/accounts/${accountId}/vehicles/${id}/records?type=mot&page_size=200`);
  ...
  return (
    <div className="rec-shell">
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">MOT</h1>
              <select className="rpt-year-select" value={year} onChange={(e) => setYear(Number(e.target.value))}>
                {yearOptions.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <p className="rec-sub">MOT test history, results, and costs for this vehicle.</p>
          </div>
          <Link href={`/dashboard/vehicles/${id}/records?type=mot`} className="rec-btn rec-btn--primary rec-btn--icon" title="Add record">+</Link>
        </div>
      </header>
      ...
      {!loading && !hasData && (
        <Card><div className="rec-empty"><p>No MOT records yet. Add records via the Records page.</p></div></Card>
      )}
