// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/cleaning/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for cleaning records on a vehicle.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/cleaning
// ============================================================
...
export default function CleaningPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      const res = await apiFetch(`/api/v1/accounts/${accountId}/vehicles/${id}/records?type=cleaning&page_size=200`);
  ...
  return (
    <div className="rec-shell">
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Cleaning</h1>
              <select className="rpt-year-select" value={year} onChange={(e) => setYear(Number(e.target.value))}>
                {yearOptions.map((y) => <option key={y} value={y}>{y}</option>)}
              </select>
            </div>
            <p className="rec-sub">Valeting and cleaning records and costs for this vehicle.</p>
          </div>
          <Link href={`/dashboard/vehicles/${id}/records?type=cleaning`} className="rec-btn rec-btn--primary rec-btn--icon" title="Add record">+</Link>
        </div>
      </header>
      ...
      {!loading && !hasData && (
        <Card><div className="rec-empty"><p>No cleaning records yet. Add records via the Records page.</p></div></Card>
      )}
