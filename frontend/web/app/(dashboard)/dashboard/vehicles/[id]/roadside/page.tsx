// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/roadside/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for roadside assistance records on a vehicle.
//   Shows annual spend, monthly bar chart, and a record list
//   for all records of type "roadside".
//
// Design:
//   Fetches records via the generic records endpoint filtered
//   by type=roadside. Client-side aggregation builds the monthly
//   breakdown and stat totals. Mirrors the expenses page pattern.
//   No inline add form; users add via the Records page.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/roadside
// ============================================================
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await apiFetch(
        `/api/v1/accounts/${accountId}/vehicles/${id}/records?type=roadside&page_size=200`
      );
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Roadside</h1>
        ...
        <>
          <Card style={{ overflow: "visible" }}>
            ...
            <div className="fuel-stats">
              ...
              <Card className="fuel-stat" padding="var(--space-4)" hoverEffect="glow"><span className="fuel-stat__value">{count}</span><span className="fuel-stat__label">Call-outs ({year})</span></Card>
              <Card className="fuel-stat" padding="var(--space-4)" hoverEffect="glow"><span className="fuel-stat__value">{count > 0 ? formatGBP(avgCost) : "-"}</span><span className="fuel-stat__label">Avg per call-out</span></Card>
