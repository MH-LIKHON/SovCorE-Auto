// ============================================================
// frontend/web/app/(dashboard)/dashboard/vehicles/[id]/repairs/page.tsx
// ============================================================
//
// Purpose:
//   Analytics page for repair records on a vehicle.
//   Shows annual spend, monthly bar chart, and a record list
//   for all records of type "repair".
//
// Design:
//   Fetches records via the generic records endpoint filtered
//   by type=repair. Client-side aggregation builds the monthly
//   breakdown and stat totals. Mirrors the expenses page pattern.
//   No inline add form; users add via the Records page.
//
// Consumed by:
//   - Routed at /dashboard/vehicles/[id]/repairs
// ============================================================
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const res = await apiFetch(
        `/api/v1/accounts/${accountId}/vehicles/${id}/records?type=repair&page_size=200`
      );
      if (res.ok) {
        ...
        <div className="rec-head__row">
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "var(--space-3)" }}>
              <h1 className="rec-title">Repairs</h1>
