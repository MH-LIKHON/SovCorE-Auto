...
//
// Purpose:
//   Account creation screen. Visually and functionally
//   identical to /login — same full-screen layout, same staged
//   flow (email → six-digit code → optional TOTP). The backend
//   auto-creates a personal account and Owner membership on
//   first code verification, so there is no separate sign-up
//   step. The page exists to give new users distinct copy and a
//   dedicated entry point.
//
// Design:
//   Uses the same LoginBackground + LoginCard + BrandLockup
//   composition as /login so the two pages are visually
//   identical. The only difference is the footer link
//   ("Already have an account? Sign in" → /login instead of
//   "No account? Create one" → /register).
//
// Consumed by:
